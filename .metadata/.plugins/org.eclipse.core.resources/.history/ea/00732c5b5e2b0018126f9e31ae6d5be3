...
bool Network::generate_network()
{
	...
	cuda::init<<1,1>>(nodeArrayPtrs, node_index, nodeDeviceArrayLengths);
	cuda::init<<1,1>>(weightArrayPtrs, weight_index, weightDeviceArrayLengths);
	cuda::init<<1,1>>(biasArrayPtrs, bias_index, biasDeviceArrayLengths);
	...
	for(int l = 0; l < no_iterations; l++)
	{
		for(int j = 0; j < outer_loop; j++)
		{
			...
			for(int i = 0; i < batch_size; i++)
			{
				...
				cuda::loadPicture<<1,1>>(nodeArrayPtrs[0], devicePictureAddr);
